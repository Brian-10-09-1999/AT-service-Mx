...
public class PreferencesVar {
    ...
    // TODO: 04/05/2017 estos metodo son para sar de pais se esta el loguion
    public static void setPaisRegion( int valor) {
        guardarPais = context.getSharedPreferences(PAIS, MODE_PRIVATE);
        SharedPreferences.Editor editor = guardarPais.edit();
        editor.putInt("numeroPais", valor);
        editor.commit();
    }
    public static int getPaisRegion() {
        SharedPreferences recuperarValor = context.getSharedPreferences(PAIS, MODE_PRIVATE);
        int valorRecuperado = recuperarValor.getInt("numeroPais", 0);

        return valorRecuperado;
    }

    // TODO: 04/05/2017 estos metodos se usan para guadar valor de localizacion
